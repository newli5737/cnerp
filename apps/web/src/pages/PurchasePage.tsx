...
export function PurchasePage() {
  ...
  const { data: partners = [] } = useQuery({
    queryKey: ['partners'],
    queryFn: () => api<any[]>('/partners?type=SUPPLIER'),
  });
    ...
    <div>
      ...
      <Table
        ...
        columns={[
          ...
          { title: t('status'), render: (_, r) => <Tag>{r.status}</Tag> },
          { title: t('amount'), render: (_, r) => Number(r.totalAmount).toLocaleString() },
          hasPermission('purchase.write')
            ? {
                title: t('actions'),
                render: (_, r) => (
                  <Space>
                    ...
                    {(r.status === 'DRAFT' || r.status === 'CONFIRMED') && (
                      <Button
                        size="small"
                        type="primary"
                        onClick={() => action.mutate({ id: r.id, act: 'receive' })}
                      >
                        {zh ? '入库' : 'Nhận hàng'}
                      </Button>
                    )}
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="partnerId" label={zh ? '供应商' : 'NCC'} rules={[{ required: true }]}>
            <Select
              ...
              <>
                {fields.map((field) => (
                  <Space key={field.key} align="baseline" style={{ display: 'flex', marginBottom: 8 }}>
                    ...
                    <Form.Item {...field} name={[field.name, 'unitCost']} rules={[{ required: true }]}>
                      <InputNumber min={0} />
                    </Form.Item>
